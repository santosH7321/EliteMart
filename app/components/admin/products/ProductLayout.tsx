...
const ProductLayout = () => {
  ...
  const { data, isLoading, mutate } = useSWR('/api/product', fetcher)
      ...
      {isLoading && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
          {Array(8).fill(0).map((_, i) => (
            <div key={i} className="rounded-2xl p-4" style={{ background: t.cardBg, border: `1px solid ${t.border}` }}>
              <Skeleton active />
            </div>
          ))}
        </div>
          ...
          {filtered.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              {filtered.map((product, i) => (
                <ProductCard key={product._id} product={product} index={i} onEdit={handleEdit} onDelete={mutate} />
              ))}
            </div>
          ) : (
